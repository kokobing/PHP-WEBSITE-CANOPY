...
if(isset($action) && $action=="02" && $onuserperm_addprem=='1'){
    ...
    header('Location:pageset.php'); exit();//跳转
}	
...
if(isset($action) && substr($action,0,2)=="04" && $onuserperm_editprem=='1'){
	...
	$objDB->Execute($strSQL);
	header('Location:pageset.php?03'.$onenewsinfoid); exit();
}	
...
function neededitupfile(getfilename){
 $.post('/admin/inc/ajax_getfiles.php',{getfilename:getfilename,datasheet:'pagesetpic'},function(data){
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td align="center"><table width="95%" border="0" cellspacing="5" cellpadding="3">
                      <tr>
              <td height="20" align="left" valign="bottom" class="txt_leftmenu"><strong>版面名称</strong></td>
		   ...
		   <?php for($i=0;$i<$intNews;$i++) {?>
            <tr>
              <td width="28%" height="20" align="left" valign="bottom" class="txt_leftmenu"><a href="pageset.php?03<?php echo $arrnewsinfos[$i][id_pageset];?>"  class="txt_leftmenu"><?php echo cutstr($arrnewsinfos[$i][title],60,1);?></a></td>
...
<div id="lineout">
   ...
   $strSQL="SELECT opicname,id_pagesetpic,type FROM pagesetpic WHERE id_pageset=$onenewsid";
   $objDB->Execute($strSQL);
